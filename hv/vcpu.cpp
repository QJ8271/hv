...
namespace hv {
	...
	static bool enable_vmx_operation(vcpu const* const cpu) {
		...
		if (!cpu->cached.cpuid_01.cpuid_feature_information_ecx.virtual_machine_extensions) {
			LOG("[hv] VMX not supported by CPUID.\n");
			return false;
		}
		...
		if (!cpu->cached.feature_control.lock_bit ||
			...
			LOG("[hv] VMX not enabled outside SMX.\n");
			return false;
		}
	...
	static bool load_vmcs_pointer(vmcs& vmcs_region) {
		...
		if (!vmx_vmclear(vmcs_phys)) {
			LOG("[hv] VMCLEAR failed.\n");
			return false;
		}

		if (!vmx_vmptrld(vmcs_phys)) {
			LOG("[hv] VMPTRLD failed.\n");
			return false;
		}
	...
	bool virtualize_cpu(vcpu* const cpu) {
		...
		LOG("[hv] Cached VCPU data.\n");

		if (!enable_vmx_operation(cpu)) {
			LOG("[hv] Failed to enable VMX operation.\n");
			return false;
		}

		LOG("[hv] Enabled VMX operation.\n");

		if (!enter_vmx_operation(cpu->vmxon)) {
			LOG("[hv] Failed to enter VMX operation.\n");
			return false;
		}

		LOG("[hv] Entered VMX operation.\n");

		if (!load_vmcs_pointer(cpu->vmcs)) {
			LOG("[hv] Failed to load VMCS pointer.\n");
			vmx_vmxoff();
			return false;
		}

		LOG("[hv] Loaded VMCS pointer.\n");

		prepare_external_structures(cpu);

		LOG("[hv] Initialized external structures.\n");

		write_vmcs_ctrl_fields(cpu);
		write_vmcs_host_fields(cpu);
		write_vmcs_guest_fields();

		LOG("[hv] Wrote VMCS fields.\n");
		...
		LOG("Launching VM on VCPU#%i...\n", KeGetCurrentProcessorIndex() + 1);

		if (!vm_launch()) {
			LOG("[hv] VMLAUNCH failed. Instruction error = %lli.\n",
		...
		if (vmx_vmcall(input) == hypervisor_signature)
			LOG("[hv] Successfully pinged the hypervisor.\n");
		...
		LOG("[hv] Measured VM-exit overhead (TSC = %zi).\n",
			cpu->vm_exit_tsc_overhead);
		LOG("[hv] Measured VM-exit overhead (MPERF = %zi).\n",
			cpu->vm_exit_mperf_overhead);
		LOG("[hv] Measured VM-exit overhead (CPU_CLK_UNHALTED.REF_TSC = %zi).\n",
